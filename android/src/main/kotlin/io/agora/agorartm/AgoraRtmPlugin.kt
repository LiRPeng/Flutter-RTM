    ...
    private fun handleClientMethod(methodName: String?, params: Map<String, Any>, result: Result) {
        ...
        when (methodName) {
            ...
            "addOrUpdateChannelAttributes" -> {
                val channelId: String = args?.get("channelId") as String
                val attributes = args?.get("attributes") as List<Map<String, String>>
                val enableNotificationToChannelMembers = args?.get("enableNotificationToChannelMembers") as Boolean
                var rtmChannelAttribute = ArrayList<RtmChannelAttribute>()
                attributes!!.forEach {
                    var rtmAttribute = RtmChannelAttribute()
                    rtmAttribute.key = it["key"]
                    rtmAttribute.value = it["value"]
                    rtmChannelAttribute.add(rtmAttribute)
                }
                val channelAttributeOptions = ChannelAttributeOptions()
                channelAttributeOptions.enableNotificationToChannelMembers = enableNotificationToChannelMembers
                client.addOrUpdateChannelAttributes(channelId, rtmChannelAttribute, channelAttributeOptions,
                        object : ResultCallback<Void> {
                            ...
                            override fun onFailure(p0: ErrorInfo) {
                                runMainThread {
                                    result.success(hashMapOf("errorCode" to p0.getErrorCode()))
                                }
                            }
            ...
            "getChannelAttributesByKeys" -> {
                ...
                client.getChannelAttributesByKeys(channelId, keys, object : ResultCallback<List<RtmChannelAttribute>> {
            ...
            "getChannelMemberCount" -> {
                val channelIds: List<String>? = args?.get("channelIds") as List<String>
                client.getChannelMemberCount(channelIds, object : ResultCallback<List<RtmChannelMemberCount>> {
                    override fun onSuccess(p0: List<RtmChannelMemberCount>?) {
                        val attributes: MutableMap<String, Int> = HashMap()
                        p0?.map {
                            attributes[it.channelID] = it.memberCount
                        }
                        runMainThread {
                            result.success(hashMapOf(
                                    "errorCode" to 0,
