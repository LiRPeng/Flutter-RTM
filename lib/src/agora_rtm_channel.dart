...
class AgoraRtmChannel {
  ...
  void Function(List<RtmChannelAttribute> attributes) onAttributesUpdated;
  ...
  _eventListener(dynamic event) {
    ...
    switch (map['event']) {
      ...
      case "onMemberCountUpdated":
        ...
        this?.onMemberCountUpdated(memberCount);
        break;
      case "onAttributesUpdated":
        String rtmChannelAttributes = map["attributes"];
        List list = json.decode(rtmChannelAttributes);
        List<RtmChannelAttribute> attributes =
            list.map((m) => new RtmChannelAttribute.fromJson(m)).toList();
